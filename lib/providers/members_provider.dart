...
class MembersProvider with ChangeNotifier {
  ...
  MembersProvider(
      {@required dataRepository, int mId = 0}) // Providing a default value
      : _dataRepository = dataRepository,
        _mId = mId {
    // Initializing _mId in the constructor
    _initializeData();
  }
  ...
  Future<bool> changeProfileData({email, password, bio, int? age}) async {
    ...
    int mId =
        await _dataRepository.changeAccountData(data: temp.toJson(), id: _mId);
    if (mId != null) {
      _members[_mId] = temp;
      notifyListeners();
      return true;
