...
class PublishesProvider with ChangeNotifier {
  ...
  PublishesProvider({@required dataRepository})
  ...
  Future<List<Author>> getBookAuthors(int bkId) async {
    ...
    //Future based
    await for (List<int> authorIds
        in _dataRepository.bookAuthorsStream(id: bkId)) {
  ...
  Future<List<Book>> getAuthorBooks(int aId) async {
    ...
    //Future based
    await for (List<int> bookIds
        in _dataRepository.authorBooksStream(id: aId)) {
